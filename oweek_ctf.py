...
async def run_ctf(user):
    ...
    if random_number == winning_number:
        ...
        await user.send(
            f"Congratulations, {user.name} 🧡! Please present this flag to the WIT stall to claim your prize: \n"
            + "`${WIT_FLAG_2023: Yay thank you for helping Willow by exploring the server!}.` \n\n Please don't share the flag with others to keep it fun for them 🤫"
        )
