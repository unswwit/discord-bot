...
class createStudySessionCog(commands.Cog):
    ...
    @app_commands.command(name="create-study-session", description="Schedule a study session and invite WIT Discord members!")
    ...
    async def create_study_session(self, inter: discord.Interaction, date: str, start_time: str, end_time: str):
        ...
        **🧡 Study Session! 🧡**\n\n<@{id}> is having a study session on {startString} until {endString} in the WIT Discord Server!\n\nClick the buttons below to RSVP.\n\n
        """
        embed = discord.Embed(title=f"Attendees (1):",
                              description=f"\n\n- <@{id}>",
                              color=discord.Color.orange())
        view = MyView(id)
        await inter.response.send_message(content=messageContent, embed=embed, view=view)
...
class MyView(discord.ui.View):
    ...
    async def updateMessage(self, inter: discord.Interaction):
        ...
        embed = discord.Embed(title=f"Attendees ({len(self.attendees)}):",
                              color=discord.Color.orange(),
                              description=f"\n\n{attendeesList}")
